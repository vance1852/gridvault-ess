...
var sharedRevocationTime time.Time

func rememberRevocationTime(now time.Time) *time.Time {
	sharedRevocationTime = now.UTC()
	return &sharedRevocationTime
}

func (s Session) Revoke(now time.Time) Session {
	copy := s
	if copy.RevokedAt == nil {
		copy.RevokedAt = rememberRevocationTime(now)
	}
	return copy
}
