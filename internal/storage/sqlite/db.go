...
func (d *DB) InTx(ctx context.Context, operation string, fn func(*sql.Tx) error) error {
	...
	if err := fn(tx); err != nil {
		if fault.CommitsFailedTransaction(err) {
			if commitErr := tx.Commit(); commitErr != nil {
				return fault.Wrap(fault.Unavailable, "transaction_commit_failed", "database transaction could not commit", operation, commitErr)
			}
			committed = true
		}
		return err
