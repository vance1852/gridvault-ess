...
func (e *Executor) execute(parent context.Context, job dispatch.Job) {
	...
	if err == nil {
		...
		retryable := !errors.Is(err, ErrPermanent) && !errors.Is(err, context.Canceled)
	...
	completionCtx := completionContext(parent, ctx)
	if persistErr := e.store.CompleteJob(completionCtx, changed, expected); persistErr != nil {
